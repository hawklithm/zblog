...
public class CategoryManager{
  ...
  public List<MapContainer> listAsTree(){
    ...
    for(int i = 1; i < preOrder.size(); i++){
      ...
      parent.putIfAbsent("nodes", new ArrayList<MapContainer>());
      List<MapContainer> list = parent.get("nodes");
      list.add(current);
    }

    List<MapContainer> reList = root.get("nodes");
    if (reList==null){
      return new ArrayList<MapContainer>();
    }
    return reList;
  }
  ...
  private static MapContainer getLastParentByLevel(MapContainer mc, int currentlevel){
    ...
    for(int i = 1; i < currentlevel; i++){
      current.putIfAbsent("nodes", new ArrayList<MapContainer>());
      List<MapContainer> children = current.get("nodes");
