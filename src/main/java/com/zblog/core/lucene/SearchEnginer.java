...
public final class SearchEnginer{
  ...
  private SearchEnginer(String indexDir){
    try{
      Directory directory = FSDirectory.open(new File(WebConstants.APPLICATION_PATH, indexDir).toPath());
      Settings settings = Settings.builder()
              .put("path.home", WebConstants.APPLICATION_PATH)
              .put("path.conf",WebConstants.APPLICATION_PATH)
              .build();
      Environment env = new Environment(settings);
      org.wltea.analyzer.dic.Dictionary.initial(new Configuration(env));
  ...
  Directory initDirectory(File path) throws IOException{
    ...
    FSDirectory dir = FSDirectory.open(path.toPath());// 装载磁盘索引
    /* RAMDirectory来访问索引其速度和效率都是非常优异的 */
    RAMDirectory map = new RAMDirectory(dir, IOContext.READ);
    NIOFSDirectory nio = new NIOFSDirectory(path.toPath());// 基于并发大文件的NIO索引
    // 组合不同Directory的优点
  ...
  public static final class SearchEnginerHolder{
